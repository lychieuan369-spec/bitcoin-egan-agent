...
def format_message(sig: dict, tf_label: str) -> str:
    ...
    lines = [
        f"{emoji} <b>BTCUSDT {tf_label} — {sig['signal']} ({sig['strength']})</b>",
        f"🕐 {now}",
        f"",
        f"💰 Giá: <b>${sig['price']:,.2f}</b>",
        f"📊 RSI(14): {sig['rsi']}",
    ...
    if sig["signal"] == "BUY":
        lines += [
            ...
            f"SL:     ${sig['stop_loss_buy']:,.2f} (-2%)",
            f"TP1:    ${sig['tp1_buy']:,.2f} (+2%)",
            f"TP2:    ${sig['tp2_buy']:,.2f} (+4%)",
        ]
        ...
        lines += [
            ...
            f"📌 <b>Kế hoạch:</b>",
            f"Entry:  ${sig['entry']:,.2f}",
            f"SL:     ${sig['stop_loss_sell']:,.2f} (+2%)",
            f"TP1:    ${sig['tp1_sell']:,.2f} (-2%)",
            f"TP2:    ${sig['tp2_sell']:,.2f} (-4%)",
        ]
...
def main():
    ...
    if not any_signal:
        print("No signals on any timeframe.")
