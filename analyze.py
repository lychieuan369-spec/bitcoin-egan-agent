#!/usr/bin/env python3
"""
Bitcoin Egan Trader - RSI(14) + EMA(9) + WMA(45) signal analyzer
...
def get_signal(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "entry": price,
        "stop_loss_buy": round(price * 0.98, 2),
        "stop_loss_sell": round(price * 1.02, 2),
        "tp1_buy": round(price * 1.02, 2),
...
def print_signal(sig: dict, symbol: str, timeframe: str):
    ...
    print(f"\n{'='*55}")
    print(f"📊 {symbol} | {timeframe.upper()} | {now}")
    print(f"{'='*55}")
    ...
    print(f"\n{signal_emoji[sig['signal']]} TÍN HIỆU: {sig['signal']} ({sig['strength']})")
    ...
    if sig["signal"] == "BUY":
        print(f"\n📌 Kế hoạch giao dịch BUY:")
        print(f"  Entry:     ${sig['entry']:,.2f}")
        print(f"  Stop Loss: ${sig['stop_loss_buy']:,.2f} (-2%)")
        print(f"  TP1:       ${sig['tp1_buy']:,.2f} (+2%, 1:1 R:R)")
        print(f"  TP2:       ${sig['tp2_buy']:,.2f} (+4%, swing)")
    elif sig["signal"] == "SELL":
        print(f"\n📌 Kế hoạch giao dịch SELL:")
        print(f"  Entry:     ${sig['entry']:,.2f}")
        print(f"  Stop Loss: ${sig['stop_loss_sell']:,.2f} (+2%)")
        print(f"  TP1:       ${sig['tp1_sell']:,.2f} (-2%, 1:1 R:R)")
        print(f"  TP2:       ${sig['tp2_sell']:,.2f} (-4%, swing)")
...
# ── Main ────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Bitcoin Egan Trader - RSI EMA WMA Signal Analyzer")
    ...
    print(f"Đang tải dữ liệu {args.symbol} ({args.timeframe})...")
